...
function loop_test(){
    ...
    ISA=$3
    DIRECTORY_NAME=$4
    sed -e "s/{package}/$PACKAGE_NAME/" $WORK_DIR/test/resources/RiscvTests.scala > $WORK_DIR/test/scala/RiscvTests.scala
    
    for INST in ${INSTS[@]}
    do
        echo $INST
        sed -e "s/{package}/$PACKAGE_NAME/" -e "s/{isa}/$ISA/" -e "s/{inst}/$INST/" $WORK_DIR/main/resources/Memory.scala > $WORK_DIR/main/scala/$DIRECTORY_NAME/Memory.scala
